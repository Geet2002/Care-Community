...
async function run() {
  ...
  // The cookie is full string like "token=eyJ...; Max-Age=..."
  // We should extract just "token=..."
  const rawCookieA = resA.headers['set-cookie'][0];
  const cookieA = rawCookieA.split(';')[0]; 

  const resB = await request('/api/auth/signup', 'POST', {username: userB, password: 'pw'});
  const rawCookieB = resB.headers['set-cookie'][0];
  ...
  console.log('Created community', commId, 'is_private:', resComm.data.is_private);

  const joinRes = await request(`/api/communities/${commId}/join`, 'POST', {}, cookieB);
  console.log('Join res:', joinRes.data);
